...
run_gpu_processing() {
    ...
    local sql_cmds="CALL gpu_buffer_init('9 GB', '9 GB');"$'\n'
    sql_cmds+=".timer on"$'\n'
...
run_gpu_execution() {
    ...
    local output
    output=$(echo "$sql_cmds" | "$DUCKDB" 2>&1) || true
...
# Ensure no sirius.cfg interferes
...
for qi in "${!QNAMES[@]}"; do
    ...
    run_gpu_execution "3mo" "$qi" "$DATA_DIR/bench_flows_dict.parquet" "$DATA_DIR/bench_entity_dict.parquet"
done
...
export SIRIUS_CONFIG_FILE="$CONFIG_PATH"
for qi in "${!QNAMES[@]}"; do
    run_gpu_execution "6mo" "$qi" "$DATA_DIR/bench_flows_dict_6mo.parquet" "$DATA_DIR/bench_entity_dict_6mo.parquet"
done
